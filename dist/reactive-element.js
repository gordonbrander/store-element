...
class RenderableElement extends HTMLElement {
    constructor() {
        super();
...
class ReactiveElement extends RenderableElement {
    html(state) {
        return y `<div>Hello World</div>`;
    }
    write(container, state, send) {
        const template = this.html(state);
        Z(template, container);
    }
}
// Create a new reactive element using an HTML template function
const react = (html) => {
    class CustomReactiveElement extends ReactiveElement {
    }
    CustomReactiveElement.prototype.html = html;
    return CustomReactiveElement;
};

export { ReactiveElement, react };
